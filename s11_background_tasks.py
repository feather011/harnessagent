#!/usr/bin/env python3
"""s11_background_tasks: 后台任务（基于 s10_task_system.py 扩展）。

相对 s10 的改动（其余不动：15 工具/task 系统/memory/压缩 pipeline/reactive/compact、
subagent/task、load_skill/skill catalog、todo_write、safe_path、UTF-8/stdin、permission、5 hook）：
- BackgroundManager（tasks/results/_ready + threading.Lock，daemon 线程执行）
- start：注册 task + 启 daemon 线程 + 立即返 bg_id（placeholder tool_result，不等命令）
- _run：跑命令 + 收集结果 + 标 status（completed/failed；异常/非零 exit_code 都 failed）
- should_run_background：bash.run_in_background 必须显式为 true 才后台（不猜）
- collect_background_results / inject_background_results：<task_notification> 格式，不复用 tool_use_id
- bash 工具 schema 加 run_in_background（boolean，默认 false）
- agent_loop 顶部 inject_background_results；循环里 execute 分支（同步 vs 后台）
- process group 隔离：Popen(start_new_session=True)；Windows 用 taskkill 兜底（无 killpg）
- 提示符 s11 >>
"""
import ast
import atexit
...
def agent_loop(messages, active_request):
    reactive_retries = 0
    while True:
        # loop 顶部：收集已完成的后台任务，注入 <task_notification>
...
if __name__ == "__main__":
    ...
    print(f"\033[36m使用模型 {MODEL}（15 工具含后台执行 + task + memory + 压缩 pipeline），输入 q / exit / 空行退出\033[0m", flush=True)
    while True:
        try:
            q = input("\033[36ms11 >> \033[0m").strip()
