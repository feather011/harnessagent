...
| | s01-s17 | harness/ |
|---|---------|----------|
| 性质 | 教学版（17 章独立文件） | Production 重写 |
| 代码量 | ~16,000 行 | ~3,800 行 + 145 测试 |
| 关系 | 从零构建，每章增量 | 借鉴设计，完全重写 |
| 工具数 | 逐步累加到 28+ | 35+（模块化注册） |
| 运行 | `python s17_goal_loop.py` | `python -m harness` |

s01-s17 完整保留，作为学习历史。harness/ 是 production 代码，不 import s01-s17。
