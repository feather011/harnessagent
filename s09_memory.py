#!/usr/bin/env python3
"""s09_memory: 跨会话记忆（基于 s08_context_compact.py 扩展）。

相对 s08 的改动（其余不动：9 工具/compaction pipeline/reactive/compact、subagent/task、
load_skill/skill catalog、todo_write、safe_path、UTF-8/stdin、permission、5 hook）：
- MemoryStore：Storage + Recall + Extraction + Consolidation 4 个子系统
- .memory/ + MEMORY.md 索引，4 种 memory type（user/feedback/project/reference）
- write_memory_file / rebuild_memory_index / select_relevant_memories / load_memories
- extract_memories（轮末调模型）+ should_store_memory（最后准入：scope=persistent、非临时、不重复）
- consolidate_memories（≥10 条调模型清理，snapshot 兜底回滚）
- load_memory 工具（TOOLS 9→10），SYSTEM 加 memory catalog + "Use load_memory 读全文"
- 协议适配：OpenAI 下 system 在 messages[0]，compact_history/reactive_compact 保留 system
  消息（否则压缩后 catalog/指令丢失）；_strip_surrogates 前移供 MemoryStore 使用
- 提示符 s09 >>
"""
import ast
import glob
...
# ============================================================ 工具集（基础 + task + load_skill + compact + load_memory）
TASK_TOOL = {
    "type": "function",
    "function": {
        "name": "task",
...
if __name__ == "__main__":
    ...
    print(f"\033[36m使用模型 {MODEL}（10 工具含 load_memory + 压缩 pipeline + memory），输入 q / exit / 空行退出\033[0m", flush=True)
    while True:
        try:
            q = input("\033[36ms09 >> \033[0m").strip()
