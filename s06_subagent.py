#!/usr/bin/env python3
"""s06_subagent: Subagent 嵌套执行（基于 s05_todo_write.py 扩展）。

相对 s05 的改动（其余不动：todo_write、safe_path、UTF-8/stdin、surrogate、permission、5 hook、
双层防御、rounds_since_todo 提醒）：
- 基础 6 工具提取为 BASE_TOOLS / BASE_HANDLERS
- TOOLS 6 → 7（加 task），TOOL_HANDLERS 加 task → run_subagent
- run_subagent: 子 agent 独立 context + 30 轮上限，父子共享 HOOKS（permission 对子同样生效）
- 子 agent 用 SUB_TOOLS（不含 task，禁止二级 delegation）、SUB_SYSTEM
"""
import ast
import glob
import json
...
# ============================================================ Subagent
# 子 agent 专用 prompt：专注单一子任务，返回简洁答案（与父 SYSTEM 不同）
SUB_SYSTEM = ("你是一个子 agent（subagent）。专注完成交给你的单一子任务，直接干活，不要解释。"
              "完成后返回简洁的最终答案。")

SUB_TOOLS = list(BASE_TOOLS)              # 子只有 6 个基础工具（不含 task，禁止二级 delegation）
SUB_HANDLERS = dict(BASE_HANDLERS)        # 子不注册 task


def run_subagent(prompt: str) -> str:
    """task 工具：用全新 context 跑一个子 agent，返回其 final text。30 轮上限。"""
    ...
    for _ in range(30):
        ...
        for call in msg.tool_calls:
            ...
            try:
                ...
                continue
            blocked = trigger_hooks("PreToolUse", name, args)   # 共享 hooks：permission 对子同样生效
            if blocked:
...
TOOLS = [*BASE_TOOLS, TASK_TOOL]                # 父 7 个
TOOL_HANDLERS = {**BASE_HANDLERS, "task": run_subagent}  # 父注册 task
...
if __name__ == "__main__":
    ...
    print(f"\033[36m使用模型 {MODEL}（7 工具含 task + hooks），输入 q / exit / 空行退出\033[0m", flush=True)
    while True:
        try:
            q = input("\033[36ms06 >> \033[0m").strip()
