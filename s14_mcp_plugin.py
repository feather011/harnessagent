#!/usr/bin/env python3
"""s14_mcp_plugin: MCP 插件（基于 s13_agent_teams.py 扩展，不卸 s13）。

相对 s13 的改动（其余不动：agent teams/cron/后台/task/memory/compact/subagent/permission/5 hook）：
- MCPClient：in-process MCP 客户端（register 存工具列表，call_tool 是调用边界，异常转 tool_result）
- connect_mcp 工具：连 server 拿工具列表，存 mcp_clients 字典（docs / deploy mock server）
- assemble_tool_pool()：base + 所有 MCP server 工具，每次 create 前重建（agent_loop 改 1 行）
- prefix 命名空间：mcp__{server}__{tool}（normalize 字符 → _，64 字符限制，防冲突）
- MCP_HOST_POLICY：host 决定 allow/confirm，不信 server 的 readOnlyHint；未配置默认 confirm
- check_permission 加 MCP policy 检查；handler 异常 catch 转 tool_result（不挂 loop）
- 提示符 s14 >>
"""
import ast
import atexit
...
def check_permission(tool_name: str, args: dict, prompt_user: bool = True) -> str | None:
    if tool_name.startswith("mcp__"):
        # host policy 决定；不信 server 的 readOnlyHint，未配置默认 confirm
...
# ============================================================ MCP 插件（s14）
class MCPClient:
    """in-process MCP 客户端：register 存工具列表，call_tool 是调用边界（异常转 tool_result）。"""
...
def assemble_tool_pool() -> tuple[list[dict], dict[str, callable]]:
    """base + 所有已连接 MCP server 的工具，每次 create 前重建。"""
    ...
    for server_name, server in mcp_clients.items():
        ...
        for tool_def in server.tools:
            ...
            # 闭包捕获当前 client 和原始工具名，防所有 lambda 指向循环最后一项
            handlers[prefixed] = (
...
def execute_named(handlers: dict, name: str, args: dict) -> str:
    """用动态 handler 表执行工具（MCP 工具也在内），异常转 tool_result 不挂 loop。"""
    handler = handlers.get(name)
    if handler is None:
        return f"Error: unknown tool '{name}'"
    try:
...
def agent_loop(messages, active_request):
    ...
    while True:
        ...
        tools, handlers = assemble_tool_pool()  # 动态工具池：base + 已连接 MCP server 工具（s14）
        try:
            resp = client.chat.completions.create(
        ...
        for call in msg.tool_calls:
            ...
            if should_run_background(name, args):
                ...
                output = execute_named(handlers, name, args)
            trigger_hooks("PostToolUse", name, args, output)
...
def build_system_prompt() -> str:
    ...
    connected = ", ".join(mcp_clients)
    if connected:
        sections.append(
            "Connected MCP servers: " + connected
            + "\n\nUse mcp__{server}__{tool} tools to interact with a connected server. "
              "Call connect_mcp to connect one."
        )
    return "\n\n".join(sections)
...
if __name__ == "__main__":
    ...
    print(f"\033[36m使用模型 {MODEL}（agent teams + MCP + cron + 后台 + task + memory + 压缩 pipeline），输入 q / exit / 空行退出\033[0m", flush=True)
    stdin_q: "queue.Queue" = queue.Queue()
    threading.Thread(target=_stdin_reader, args=(stdin_q,), daemon=True).start()
    SCHEDULER.start()
    had_teammates = False
    try:
        print("s14 >> ", end="", flush=True)
        while True:
